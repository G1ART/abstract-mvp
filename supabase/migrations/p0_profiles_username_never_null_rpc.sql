-- P0: Ensure RPCs never set profiles.username to NULL (fix 23502 from PATCH/payload).
-- upsert_my_profile: only set username when p_base supplies a non-empty value; else keep existing.
...
begin
  ...
  -- Only accept username from payload when it is non-empty; never set to null.
  v_username := case
    when (p_base ? 'username') and nullif(trim(lower(p_base->>'username')), '') is not null
    then nullif(trim(lower(p_base->>'username')), '')
    else null  -- signal: do not update username
  end;
