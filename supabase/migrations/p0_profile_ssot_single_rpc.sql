-- P0: Enforce single SSOT write path via RPC upsert_my_profile
-- Notes:
-- - This does NOT touch username at all (unless p_base contains 'username' for onboarding).
-- - It updates base fields + merges profile_details jsonb + updates completeness atomically.
-- - If your profiles.username column is NOT NULL without a default, inserting new profiles rows will fail
--   unless username is set elsewhere (trigger/default). In that case, ensure username generation exists.
