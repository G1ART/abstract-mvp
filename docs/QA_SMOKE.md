...
| `supabase/migrations/20260609000000_artwork_passport_enum_cast_hotfix.sql` (Sprint 6 hotfix) | Re-emits `get_artwork_passport_for_viewer` so the visibility gate uses `coalesce(v_aw.visibility::text, '')` instead of `coalesce(v_aw.visibility, '')`. The bare form forces Postgres to cast `''` to the `artwork_visibility` enum, raising `invalid input value for enum artwork_visibility: ""` on every viewer call. | Without this: every artwork detail click (logged in or out, follower or stranger) fails with 22P02 and the artwork page renders the raw error. **Apply immediately after Sprint 6 SQL.** |
